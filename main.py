...
# 환경변수 로딩 함수
def get_env_variable(name: str, required: bool = True, cast_to=int):
    ...
    try:
        ...
        raise ValueError(f"Environment variable {name} is invalid. Expected {cast_to.__name__}, got {value}")

# 환경변수 로드
TOKEN = os.getenv("TOKEN")
OWNER_ID = get_env_variable("OWNER_ID")
GUILD_ID = get_env_variable("GUILD_ID")
WELCOME_CHANNEL_ID = get_env_variable("WELCOME_CHANNEL_ID")
NOTICE_CHANNEL_ID = get_env_variable("NOTICE_CHANNEL_ID")
EVENT_CHANNEL_ID = get_env_variable("EVENT_CHANNEL_ID")
CHAT_CHANNEL_ID = get_env_variable("CHAT_CHANNEL_ID")

T1_ROLE_ID = get_env_variable("T1_ROLE_ID")
T4_ROLE_ID = get_env_variable("T4_ROLE_ID")
MANAGER_ROLE_ID = get_env_variable("MANAGER_ROLE_ID")
KOREAN_ROLE_ID = get_env_variable("KOREAN_ROLE_ID")
ENGLISH_ROLE_ID = get_env_variable("ENGLISH_ROLE_ID")
...
async def on_interaction(interaction):
    if interaction.data["component_type"] == 2:
        ...
        if cid == "t1":
            ...
            await member.add_roles(guild.get_role(T1_ROLE_ID))
            await interaction.channel.send(f"{member.mention} 님이 'T4' 역할을 신청했습니다.")
            await interaction.response.send_message("T1~T3 역할이 임시 부여되었습니다!", ephemeral=True)

        elif cid == "manager":
            await member.add_roles(guild.get_role(T1_ROLE_ID))
            await interaction.channel.send(f"{member.mention} 님이 'Manager' 역할을 신청했습니다.")
            await interaction.response.send_message("T1~T3 역할이 임시 부여되었습니다!", ephemeral=True)
